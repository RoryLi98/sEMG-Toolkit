...
class SurfaceEMG:
    ...
    def __init__(self):
        ...
        self.simulation_time: int = 10  # 模拟的总持续时间(s)
        self.sampling_rate: int = 2000  # 模拟的采样频率(Hz)
        self.ramp: np.array = np.array([1, 8, 1])  # 激发上升、稳定和下降时间(s)
        self.maximum_excitation_level: int = 20  # 运动单位的最大激发水平(%)
        self.signal_to_noise_ratio_dB: float = (
            3  # 添加到信号中的高斯噪声的信噪比(dB),0代表不添加噪声信号
        )
        self.signal_amplitude_offset: float = 0  # 添加信号幅度的偏移值(mV)

        ### Motorneuron parameters 运动神经元参数
        self.number_of_motor_units: int = 10  # 模拟中的运动单位总数
        self.recruitment_range: int = 8  # 招募阈值值范围的期望最大值
        self.excitatory_gain: int = 1  # 运动神经元兴奋性输入和其产生的发放率之间增益
        self.minimum_firing_rate: int = 5  # 最小的发放频率(Hz)
        self.peak_firing_rate_first_unit: int = 30  # 第一个运动神经元的峰值发放率(Hz)
        self.peak_firing_rate_difference: int = (
            10  # 第一个和最后一个运动神经元之间期望的峰值发放率(Hz)
        )
        self.inter_spike_interval_coefficient_variation: int = (
            0.1  # 两个动作电位之间时间间隔的方差系数
        )
        self.conduction_velocity_min_value = 4  # 最低传导速度
        self.conduction_velocity_max_value = 6  # 最大传导速度

        ### Number of fibres parameters 单位运动单元的纤维参数
        self.twitch_force_range: int = (
            20  # 肌肉纤维发生颤动所需力(unit force),即最小运动单位的肌肉收缩力量
        )
        self.motor_unit_density: int = (
            20  # 肌肉组织中每平方毫米内的运动单位纤维数量(units/mm^2).
        )
        self.smallest_motor_unit_number_of_fibres: int = (
            25  # 最小的运动单位所支配的纤维数
        )
        self.largest_motor_unit_number_of_fibres: int = (
            2725  # 最大的运动单位所支配的纤维数
        )
        self.muscle_fibre_diameter: float = 46e-3  # 肌肉纤维直径(mm)
        self.muscle_cross_sectional_diameter: int = 15  # 肌肉横截面直径(mm)
        self.maximum_number_of_motor_units: int = (
            300  # 最大MU数，用于计算MU半径和单位MU中的纤维数
        )
        self.motor_unit_depth: int = 10  # MU的深度(mm)

        ### Plot parameters 画图参数
        self.electrodes_in_z: int = 3  # 在z方向上的电极数量
        self.electrodes_in_x: int = 2  # 沿纤维方向的电极数量
        self.y_limit_minimum: float = -1  # 绘图y轴的最小值
        self.y_limit_maximum: float = 1  # 绘图y轴的最大值
        self.simulations: list = []  # 用于存储模拟结果的空列表
        self.time_start: float = 7.25  # 绘图的起始时间(s)
        self.time_end: float = 7.3  # 绘图的结束时间(s)
        self.amplitude_start: float = -20  # 绘图的起始振幅(mV)
        self.amplitude_end: float = 60  # 绘图的结束振幅(mV)
    ...
    def simulate_recruitment_model(self):
        ...
        inter_spike_interval_coefficient_variation = (
            self.inter_spike_interval_coefficient_variation
        )

        ### Time vector
        time_array = np.linspace(
            0, simulation_time, simulation_time * sampling_rate
        )  # 模拟总采样点数
        self.time_array = time_array

        ### Calculate the recruitment threshold excitation. Equation (1) in Fuglevand 1993.
        a = (
            np.log(recruitment_range) / number_of_motor_units
        )  # Constant related to eq. (1).
        recruitmenexcitatory_drive_thresholdold_excitation = np.exp(
            a * (np.arange(1, number_of_motor_units + 1, 1))
        )
        ...
        peak_firing_rate_i = (
            ...
            * (
                recruitmenexcitatory_drive_thresholdold_excitation
                / recruitmenexcitatory_drive_thresholdold_excitation[-1]
            )
        )
        ...
        maximum_excitation = (
            recruitmenexcitatory_drive_thresholdold_excitation[-1]
            + (peak_firing_rate_i[-1] - minimum_firing_rate) / excitatory_gain
        )

        ### Define the excitatory drive function 生成一个类梯形的数列.
        excitatory_drive_function = np.concatenate(
            (
                np.linspace(
                    0,
                    maximum_excitation * (maximum_excitation_level / 100),
                    ramp[0] * sampling_rate,
                ),
                np.ones(ramp[1] * sampling_rate)
                * maximum_excitation
                * (maximum_excitation_level / 100),
                (
                    np.flip(
                        np.linspace(
                            0,
                            maximum_excitation * (maximum_excitation_level / 100),
                            ramp[2] * sampling_rate,
                        )
                    )
                ),
            )
        ...
        ### Initialize the firing times for each motoneuron 建立MU个空列表来存储该MU的发放时间.
        ...
        for i in range(number_of_motor_units):
            ...
            excitatory_drive_threshold = (
                excitatory_drive_function
                - recruitmenexcitatory_drive_thresholdold_excitation[i]
            )

            # Find the samples that are associated with firing. Above this thresh => fire 看有没有超过激发的阈值
            find_excitatory_drive_threshold = np.where(excitatory_drive_threshold >= 0)[
                0
            ]
            ...
            # Calculate the time of the first impulse 第一个超阈值的时间点.
            firing_times_motor_unit[i].append(
                time_array[find_excitatory_drive_threshold[0]]
            )
            # firing_times_motor_unit[i] = np.append(firing_times_motor_unit[i], time_array[find_excitatory_drive_threshold[0]])

            # Calculate points of exceeded threshold 超出的值
            excitation_difference = excitatory_drive_threshold[
                find_excitatory_drive_threshold[0]
            ]
            ...
            ## Iterate until the current time is greater than the last sample point associated with firing 循环至低于阈值点.
            while time_instance <= time_array[find_excitatory_drive_threshold[-1]]:
                # Calculate the interspike interval.
                inter_spike_interval = max(
                    1 / (excitatory_gain * excitation_difference + minimum_firing_rate),
                    1 / peak_firing_rate_i[i],
                )

                firing_times_motor_unit[i].append(
                    firing_times_motor_unit[i][iteration_variable]
                    + (
                        inter_spike_interval_coefficient_variation
                ...
                minimum_time_index = np.argmin(
                    np.abs(
                        firing_times_motor_unit[i][iteration_variable]
                        - time_array[find_excitatory_drive_threshold]
                    )
                )

                # Update the thresholded excitatory drive.
                excitation_difference = excitatory_drive_threshold[
                    find_excitatory_drive_threshold[minimum_time_index]
                ]
            ...
            scipy.io.savemat(
                "recrtuitment_model"
                + "_"
                + str(number_of_motor_units)
                + "MUs_"
                + str(sampling_rate)
                + "sf"
                + ".mat",
                {"RM": firing_times_motor_unit},
            )
    ...
    def plot_recruitment_model(self):
        ...
        ### A list of different colors for each motor unit.
        ...
        num_colors = self.number_of_motor_units
        ...
        plt.savefig(
            "recruitment_model_{}MUs.png".format(number_of_motor_units), dpi=600
        )
    ...
    def caculate_surface_emg(self):
        ...
        b = (
            np.log(twitch_force_range) / max_number_of_motor_units
        )  # Constant related to eq. (13).
        peak_twitch_force = np.exp(
            b * (np.arange(1, max_number_of_motor_units + 1, 1))
        )  # Pi, where i = np.arange(1, number_of_motor_units + 1, 1)
        ...
        max_number_of_fibres = (
            nf_tot / total_peak_twitch_forces
        ) * peak_twitch_force  # nf_i
        if (
            max_number_of_fibres[0] < smallest_motor_unit_number_of_fibres
            or max_number_of_fibres[-1] > largest_motor_unit_number_of_fibres
        ):
        ...
        for i in range(number_of_motor_units):
            radial_position_motor_unit = (
                muscle_cross_sectional_diameter / 2 * np.random.random()
            )
        ...
        conduction_velocity_equal_steps = np.linspace(
            conduction_velocity_min_value, conduction_velocity_max_value, num_elements
        )
        conduction_velocity_random = np.random.uniform(
            conduction_velocity_min_value, conduction_velocity_max_value, num_elements
        )  # 随机取MU个数的随机值作为传到速度
        ...
        for m, element in enumerate(firing_times_motor_unit):
            ...
            simulation = np.full(
                (len(current_motor_unit), len(time_array)), current_motor_unit[0, 0]
            )
            ...
            for e in range(len(element)):
                ...
                if (
                    simulation[
                        :, time_index : time_index + current_motor_unit.shape[1]
                    ].shape
                    >= current_motor_unit.shape
                ):
                    simulation[
                        :, time_index : time_index + current_motor_unit.shape[1]
                    ] += current_motor_unit
    ...
    def simulate_surface_emg(self):
        ...
        if signal_to_noise_ratio_dB == 0:
            ...
            electrode_sum_with_noise = np.zeros(
                (number_of_electrodes_z * number_of_electrodes_x, len(time_array))
            )
            ...
            for ne in range(len(electrode_sum)):
                ...
                noise_level = (
                    np.mean(motor_unit_sum)
                    / (10 ** (signal_to_noise_ratio_dB / 20))
                    * 1000
                )
                noise = signal_amplitude_offset + noise_level * np.random.normal(
                    size=len(time_array)
                )
            ...
            plt.plot(
                time_array,
                electrode_sum_with_noise[electrod_postion - 1, :] * 10**3,
                label="sEMG with noise",
            )
            plt.plot(
                time_array,
                electrode_sum[electrod_postion - 1, :] * 10**3,
                label="Original sEMG",
            )
            plt.legend()
            ...
            plt.plot(
                time_array, electrode_sum_with_noise[electrod_postion - 1, :] * 10**3
            )
            ...
            fig.suptitle(
                "sEMG Signal with and without Noise", fontsize=20, fontweight="bold"
            )

            # Set the desired resolution
            plt.savefig(
                "sEMG_with_and_without_noise_{}MUs_{}X{}.png".format(
                    number_of_motor_units,
            ...
            scipy.io.savemat(
                ...
                + str(number_of_motor_units)
                + "MUs_"
                + str(number_of_electrodes_z)
                + "X"
                + str(number_of_electrodes_x)
                + ".mat",
                {"output": electrode_sum},
            )
    ...
    def plot_suface_emg_array_no_noise(self):
        ...
        normalized_simulation = (
            normalized_simulation - normalized_simulation.mean()
        ...
        plt.suptitle(
            "The Surface Electromyography Signal Array", fontsize=20, fontweight="bold"
        )
        ...
        # Set the desired resolution
        plt.savefig(
            "sEMG_electrode_array_without_noise_{}MUs_{}X{}.png".format(
                number_of_motor_units, number_of_electrodes_z, number_of_electrodes_x
            ),
            dpi=600,
        )
    ...
    def plot_suface_emg_array(self):
        ...
        ### Plot the normalized motor unit action potential
        normalized_simulation = simulations  # simulations np.sum(simulations, axis=0)
        normalized_simulation = (
            normalized_simulation - normalized_simulation.mean()
        ) / (normalized_simulation.max() - normalized_simulation.min())
        ...
        # Set the desired resolution
        plt.savefig(
            "sEMG_electrode_array_{}MUs_{}X{}.png".format(
                number_of_motor_units, number_of_electrodes_z, number_of_electrodes_x
            ),
            dpi=600,
        )
    ...
    def plot_one_electrode_surface_emg_no_noise(self):
        ...
        plt.plot(
            time_array, electrod_one_sum * 10**3
        ...
        # Set the desired resolution
        plt.savefig(
            "sEMG_one_electrode_wihout_noise_{}MUs_{}X{}.png".format(
                number_of_motor_units, electrodes_in_z, electrodes_in_x
            ),
            dpi=600,
        )
    ...
    def plot_one_electrode_surface_emg(self):
        ...
        plt.plot(
            time_array, surface_emg[electrod_postion - 1, :] * 10**3
        )  # Plot the electrode row postion with m numbers of motor units.
        ...
        # Set the desired resolution
        plt.savefig(
            "sEMG_one_electrode_{}MUs_{}X{}.png".format(
                number_of_motor_units, electrodes_in_z, electrodes_in_x
            ),
            dpi=600,
        )
    ...
    def subplot_one_electrode_surface_emg(self):
        ...
        # Set the desired resolution
        plt.savefig(
            "sEMG_subplot_one_electrode_{}MUs_{}X{}.png".format(
                number_of_motor_units, electrodes_in_z, electrodes_in_x
            ),
            dpi=600,
        )
