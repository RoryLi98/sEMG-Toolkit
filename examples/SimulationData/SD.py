...
class SaveData:
    ...
    def plot_saved_motor_unit(self, motor_unit_array):
        ...
        normalized_motor_unit = (
            normalized_motor_unit - normalized_motor_unit.mean()
        ) / (normalized_motor_unit.max() - normalized_motor_unit.min())
        ...
        array_size = np.arange(
            1, (number_of_electrodes_z * number_of_electrodes_x) + 1, 1
        )
        zeros_array = np.zeros(len(array_size))
        array_size_x = np.arange(
            0, number_of_electrodes_z * number_of_electrodes_x, number_of_electrodes_x
        )
        ...
        for i in range(len(array_size)):
            ax = plt.subplot(
                number_of_electrodes_z, number_of_electrodes_x, array_size[i]
            )
            ...
            for j in range(i):
                if i == array_size_x[j]:
                    ax.set_ylabel(
                        array_size[j], rotation=0, ha="center", va="center", fontsize=15
                    )
        ...
        if number_of_electrodes_z > 1:
            fig1.supylabel(
                "Motor Unit Action Potential\n Electrodes in the z direction, i.e. along the fibre",
                ha="center",
                va="center",
            )
        else:
    ...
    def plot_saved_surface_emg_array_no_noise(self, surface_emg_array):
        ...
        electrode_sum = np.zeros(
            (number_of_electrodes_z * number_of_electrodes_x, len(time_array))
        )
        ...
        normalized_simulation = y_limit_minimum + (
            (normalized_simulation - normalized_simulation.min())
            * (y_limit_maximum - y_limit_minimum)
        ) / (normalized_simulation.max() - normalized_simulation.min())
    ...
    def plot_saved_surface_emg_array(self, surface_emg_array):
        ...
        for i in range(len(array_size)):
            ...
            plt.ylim(y_limit_minimum, y_limit_maximum)
        plt.suptitle(
            "The Surface Electromyography Signal", fontsize=20, fontweight="bold"
        )
    ...
    def plot_saved_one_electrode_surface_emg_no_noise(
        self, surface_emg, number_of_electrodes_z, number_of_electrodes_x
    ):
        ### Default arguments:
        simulation_time = self.simulation_time
        sampling_rate = self.sampling_rate
        ...
        plt.plot(
            time_array, electrod_one_sum * 10**6
        )  # Plot the electrode row postion with m numbers of motor units.
    ...
    def plot_saved_one_electrode_surface_emg(
        self, surface_emg, number_of_electrodes_z, number_of_electrodes_x
    ):
        ### Default arguments:
        simulation_time = self.simulation_time
        ...
        ### Plot the simulations for each motor unit after sum
        plt.plot(
            time_array, surface_emg[electrod_postion - 1, :] * 10**6
        )  # Plot the electrode row postion with m numbers of motor units.
